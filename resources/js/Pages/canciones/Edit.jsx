...
    const añadirUsuario = (user) => {
        if (!usuariosSeleccionados.some(selected => selected.id === user.id)) {
            const nuevosUsuariosSeleccionados = [...usuariosSeleccionados, user];
...
            return;
        }
        const nuevosUsuariosSeleccionados = usuariosSeleccionados.filter(user => user.id !== userId);
        setUsuariosSeleccionados(nuevosUsuariosSeleccionados);
        setData('userIds', nuevosUsuariosSeleccionados.map(u => u.id));
...
                setMostrarUsuariosIniciales(!term.trim() && usuariosDisponibles.length > 0);
            } catch (error) {
                setResultadosBusqueda([]);
                setMostrarUsuariosIniciales(false);
            } finally {
...
    useEffect(() => {
        setResultadosBusqueda(prevResults =>
            prevResults.filter(user => !usuariosSeleccionados.some(selected => selected.id === user.id))
        );
    }, [usuariosSeleccionados]);

    useEffect(() => {
        if (!terminoBusqueda.trim() && !mostrarUsuariosIniciales) {
            ejecutarBusqueda('');
        }
        return () => {
            ejecutarBusqueda.cancel();
        };
    }, []);
...
                                                onFocus={() => {if(!terminoBusqueda) ejecutarBusqueda('')}}
...
                                                        <li key={user.id} className="px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 flex justify-between items-center cursor-pointer" onClick={() => añadirUsuario(user)}>
                                                            <div>
...
                                                            <button
                                                                type="button"
                                                                onClick={(e) => { e.stopPropagation(); añadirUsuario(user); }}

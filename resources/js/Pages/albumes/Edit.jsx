...
export default function Edit({ auth, album, errors: erroresSesion, success: mensajeExitoSesion }) {

    const initialUserIds = album.usuarios?.map(u => u.id) || [];
    const initialSelectedUsers = album.usuarios?.map(u => ({ id: u.id, name: u.name, email: u.email })) || [];
    ...
    const manejarEnvio = (e) => {
        e.preventDefault();
        post(route('albumes.update', album.id), {
            forceFormData: true,
            onSuccess: () => {
            ...
            onError: (errores) => {
                console.error("Errores de actualización de album:", errores);
            },
            preserveScroll: true,
        });
    };
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Editar Album: {album.nombre}</h2>}
        >
            <Head title={`Editar ${album.nombre}`} />
                                    ...
                                    <InputLabel htmlFor="imagen_nueva" value="Imagen (Opcional: reemplazar actual)" />
                                     {album.imagen && !data.eliminar_imagen && (
                                        <div className="mt-2 mb-4">
                                            ...
                                            <img src={`/storage/${album.imagen}`} alt="Imagen actual de la album" className="h-24 w-24 object-cover rounded border border-gray-200" />
                                        </div>
                                    )}
                                    <input
                                        type="file"

...
export default function Edit({ auth, album, errors: erroresSesion, success: mensajeExitoSesion }) {
    ...
    const idsInicialesUsuarios = album.usuarios?.map(u => u.id) || [];
    const usuariosSeleccionadosIniciales = album.usuarios?.map(u => ({ id: u.id, name: u.name, email: u.email })) || [];
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Editar Álbum: {album.nombre}</h2>}
        >
                            ...
                            <form onSubmit={manejarEnvio} className="space-y-6">
                                ...
                                <div>
                                    ...
                                    {album.imagen && !data.eliminar_imagen && (
                                        <div className="mt-2 mb-4">
                                            ...
                                            <img src={`/storage/${album.imagen}`} alt="Imagen actual del álbum" className="h-24 w-24 object-cover rounded border border-gray-200" />
                                        </div>
                                    )}
                                    <input
                                        type="file"
                                ...
                                <div className="flex items-center gap-4">
                                    <PrimaryButton disabled={processing}>
                                        {processing ? 'Actualizando...' : 'Actualizar Álbum'}
                                    </PrimaryButton>
